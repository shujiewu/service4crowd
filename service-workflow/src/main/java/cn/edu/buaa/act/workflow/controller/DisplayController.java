...
import cn.edu.buaa.act.workflow.service.impl.BpmnDisplayJsonConverter;
import cn.edu.buaa.act.workflow.service.impl.PermissionService;
import cn.edu.buaa.act.workflow.util.mapper.*;
import cn.edu.buaa.act.workflow.util.mapper.InfoMapper;
...
public class DisplayController {
    ...
    @RequestMapping(value = "/model/{processModelId}/modelJson", method = RequestMethod.GET, produces = "application/json")
    ...
    @RequestMapping(value = "/processInstance/history/{processInstanceId}/modelJson", method = RequestMethod.GET, produces = "application/json")
...
//        }
        ...
        Map<String,List<String>> completedActivityInstances = new HashMap<>();
        ...
        if (CollectionUtils.isNotEmpty(activityInstances)) {
            for (HistoricActivityInstance activityInstance : activityInstances) {
                if (activityInstance.getEndTime() != null) {
                    completedActivityInstances.computeIfAbsent(activityInstance.getActivityId(), k -> new ArrayList<>());
                    completedActivityInstances.get(activityInstance.getActivityId()).add(activityInstance.getTaskId());
                } else {
                    currentActivityinstances.add(activityInstance.getActivityId());
                }
            }
        }
        ObjectNode displayNode = processElementsNew(pojoModel, completedActivityInstances, currentActivityinstances);
        return displayNode;
    }
    @RequestMapping(value = "/processInstance/{processInstanceId}/modelJson", method = RequestMethod.GET, produces = "application/json")
    public JsonNode getModelRuntimeJSON(@PathVariable String processInstanceId) {
...
//        }
    ...
    protected ObjectNode processElementsNew(BpmnModel pojoModel, Map<String,List<String>> completedElements, Set<String> currentElements) {
        ObjectNode displayNode = objectMapper.createObjectNode();
        ...
        for (org.activiti.bpmn.model.Process process : pojoModel.getProcesses()) {
            processElementsNew(process.getFlowElements(), pojoModel, elementArray, flowArray, diagramInfo, completedElements, currentElements);
    ...
    protected ObjectNode processProcessElements(BpmnModel pojoModel, Map<String,String> completedElements, Set<String> currentElements) {
        ObjectNode displayNode = objectMapper.createObjectNode();
        GraphicInfo diagramInfo = new GraphicInfo();
    ...
    protected void processElementsNew(Collection<FlowElement> elementList, BpmnModel model, ArrayNode elementArray, ArrayNode flowArray, GraphicInfo diagramInfo, Map<String,List<String>> completedElements,
        ...
        for (FlowElement element : elementList) {
            ...
            if (completedElements != null) {
                elementNode.put("completed", completedElements.containsKey(element.getId()));
                if(completedElements.containsKey(element.getId())){
                    StringBuilder taskIdList = new StringBuilder();
                    for(int i=0;i<completedElements.get(element.getId()).size();i++){
                        taskIdList.append(completedElements.get(element.getId()).get(i)).append(";");
                    }
                    elementNode.put("taskId",taskIdList.toString());
                }
                else
                {
                    elementNode.put("taskId","");
                }
            }
            ...
            if (element instanceof SequenceFlow) {
                ...
                if (element instanceof SubProcess) {
                    ...
                    processElementsNew(subProcess.getFlowElements(), model, elementArray, flowArray, diagramInfo, completedElements, currentElements);
    ...
    protected void processElements(Collection<FlowElement> elementList, BpmnModel model, ArrayNode elementArray, ArrayNode flowArray, GraphicInfo diagramInfo, Map<String,String> completedElements,
                                   Set<String> currentElements) {
        ...
        for (FlowElement element : elementList) {
            ...
            if (completedElements != null) {
                elementNode.put("completed", completedElements.containsKey(element.getId()));
                elementNode.put("taskId",completedElements.get(element.getId()));
            }
