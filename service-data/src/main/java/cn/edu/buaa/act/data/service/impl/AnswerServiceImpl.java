...
import cn.edu.buaa.act.data.repository.AnswerReposiory;
...
public class AnswerServiceImpl implements IAnswerService {
    ...
    public String queryAnswerEntityType(String answerEntityId) {
        return answerReposiory.findById(answerEntityId).get().getDataType();
    }
    @Override
    public void deleteAnswerEntity(String id) {
        answerReposiory.deleteById(id);
    }
    @Override
    public AnswerEntity queryById(String id) {
        return answerReposiory.findById(id).get();
    }
    @Override
    ...
    public AnswerStatRepresentation insertDataStat(String dataId) {
        ...
        if (answerEntity.getDataType().equals("continuous")) {
            ...
            items.forEach(item -> {
                ...
                if (size % 2 == 0) {
                    mid = (answerValue.get(size / 2) + answerValue.get(size / 2 + 1)) / 2;
                } else {
                    mid = answerValue.get(size / 2);
                }
